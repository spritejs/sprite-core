...
Object.assign(_basesprite.default.prototype, {
  resolveStates: function resolveStates(states, before, after) {
    ...
    var _resolveStates = function _resolveStates() {
      _this.__ignoreAction = false;

      var fromState = _this.attr('state');
      ...
      var resolveState = function resolveState(state, i) {
        var promise = new Promise(function (resolve) {
          _this.once("state-to-".concat(state), function () {
            fromState = state;

            if (i === len - 1) {
              // lastState
              delete _this[_resolveState];
            }

            if (after) after.call(_this, states);
            resolve(_this);
          });

          _this.once("state-from-".concat(fromState), function (_ref) {
    ...
    if (rs) {
      ...
      var promise = rs.promise.then(function () {
        if (before) before.call(_this, states);
  ...
  show: function show() {
    ...
    if (states.show) {
      ...
      var deferred = this.resolveStates(_st, function () {
        var state = _this2.attr('state');

        if (state === 'hide') {
          _this2.once('state-from-hide', function () {
            _this2.attr('display', originalDisplay);
          });
        }
      });
      deferred.promise = deferred.promise.then(function () {
        if (!_this2[_hide]) {
          delete _this2.__attr.__originalDisplay;
          delete _this2.__attr.__originalState;

          if (states.show.__default) {
            delete states.show;

            _this2.attr('states', states);
          }
        }
    ...
    if (rs) {
      ...
      rs.promise.then(function () {
        _this2.attr('state', originalState);
        ...
        if (!states.show) {
          ...
          if (states.beforeShow) {
            Object.keys(states.beforeShow).forEach(function (key) {
              beforeHide[key] = _this3.attr(key);
            });
          }
      ...
      var deferred = this.resolveStates(['beforeEnter', 'afterEnter'], function (_states) {
        ...
        if (state !== 'beforeEnter' && state !== 'afterEnter' && (!states.afterEnter || states.afterEnter.__default)) {
          ...
          Object.keys(states.beforeEnter).forEach(function (key) {
            afterEnter[key] = _this4.attr(key);
          });
  ...
  exit: function exit(toState) {
    ...
    var _exit = function _exit() {
      ...
      if (states && (states.beforeExit || states.afterExit)) {
        ...
        var deferred = _this5.resolveStates(['beforeExit', 'afterExit'], function () {
          state = _this5.attr('state');
        ...
        deferred.promise.then(function () {
          if (!onbyone) {
            _this5.attr(afterEnter);

            _this5.__attr.quietSet('state', toState || state);
          }
        ...
        if (rs) {
          ...
          rs.promise.then(function () {
            ...
            return _exit2.call(_this5);
          });
          ret = rs;
        } else {
          ret = _exit2.call(_this5);
      ...
      if (_this5.children) {
        var exitMode = _this5.attr('exitMode');

        if (exitMode === 'onebyone' || exitMode === 'onebyone-reverse') {
          var promise = Promise.resolve(_this5);
          var resolved = false;
          ...
          promise = promise.then(function () {
            var p = ret.promise || Promise.resolve(_this5);
            currentTask = ret;
            return p.then(function () {
              _this5.children.forEach(function (c) {
                var states = c.attr('states');
        ...
        var _deferred2 = doActions(ret, _this5, 'exit');
